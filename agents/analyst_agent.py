...
from tools.stock_tools import get_stock_price, compare_stocks, get_company_info
from tools.technical_tools import get_technical_indicators, get_price_history
load_dotenv()
llm = ChatOpenAI(model='gpt-4o', temperature=0.2, openai_api_key=os.getenv('OPENAI_API_KEY'))
ANALYST_TOOLS = [get_stock_price, compare_stocks, get_technical_indicators, get_price_history, get_company_info]

class AnalystAgent:

    def __init__(self):
        self.graph = create_agent(model=llm, tools=ANALYST_TOOLS, system_prompt=ANALYST_SYSTEM_PROMPT)
